...
mod tests {
    ...
    fn test_resource(name: &str) -> PathBuf {
        PathBuf::from(format!("{}{}{}", env!("CARGO_MANIFEST_DIR"), "/tests/resources/espidf/components/", name))
    }
    ...
    fn test_get_component_metadata() {
        ...
        assert_eq!(get_component_metadata(&test_resource("")).unwrap(), Some(Metadata {
            description: "mDNS".to_string(),
            version: "1.1.0".to_string(),
            dependencies: BTreeMap::from([("idf".to_string(), Dependency { version: ">=5.0".to_string() })]),
        }));
    }
    ...
    fn test_version_checking() {
        ...
        assert!(component_exists_and_matches(&VersionReq::parse(">=1.1.0").unwrap(), &valid_path).unwrap());

        // Check a non-matching version
        assert!(!component_exists_and_matches(&VersionReq::parse(">=1.2.0").unwrap(), &valid_path).unwrap());
    }
}